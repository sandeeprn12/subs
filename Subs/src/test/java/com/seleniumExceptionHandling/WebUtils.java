...
class WebUtils {
	...
	public void javaScript_ScrollIntoMIDDLEView_AndHighlight(Object element) {
		try {
			...
			((JavascriptExecutor) DriverFactory.getDriver()).executeScript(""
					...
					+ "",elem);
			((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].style='';"
					+ "arguments[0].style='background-color:Yellow;border-bottom: 1px solid red;border-top: 1px solid red;color: red';", elem);
		} catch (Exception e) {
			//new CustomExceptionHandler(e);
	...
	public void javaScript_ScrollIntoBOTTOMView_AndHighlight_ThenClick(Object element,String desc) {
		...
		try {
			...
			((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].style='background-color:Yellow;border-bottom: 1px solid red;border-top: 1px solid red;color: red;'", elem);
			((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].click()", elem);
			SnapshotManager.takeSnapShot(methodName);
		} catch (Exception e) {
			new CustomExceptionHandler(e);
